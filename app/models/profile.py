"""
Pydantic models for the profile
"""
...
class ProfileBase(BaseModel):
    ...
    address: Optional[str] = Field(
        None, description="The address of the profile")
    ...
    zipcode: Optional[str] = Field(
        None, description="The zipcode of the profile")
    available: Optional[bool] = Field(
        True, description="Indicate if the profile is available to be friend")


class ProfileResponse(ProfileBase):
    """
    Response model for the profile
    """
    model_config = ConfigDict(from_attributes=True)

    id: int = Field(..., description="The ID of the profile")
    created_at: datetime = Field(...,
                                 description="Date/Time when profile was created")
    updated_at: datetime = Field(...,
                                 description="Date?Time for the last profile update")


class PaginatedProfileResponse(BaseModel):
    """
    Response model for profiles list using pagination
    """
